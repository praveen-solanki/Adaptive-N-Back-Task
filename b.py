...
class NBackApp(ctk.CTk):
    ...
    def __init__(self, n_value=1):
        ...
        # CSV for recording metrics
        self.csv_file_path = os.path.abspath("metrics.csv")
        self.csv_file = open(self.csv_file_path, "w", newline="")
        self.csv_writer = csv.writer(self.csv_file)
        self.csv_writer.writerow(['accuracy', 'reaction_time', 'error_ratio'])
    ...
    def draw_shape(self, shape_type, size, angle, color):
        """Displays the N value at a random position with a glowing effect."""
        canvas_width = max(self.canvas.winfo_width(), 200)  # Prevents zero or negative width
        canvas_height = max(self.canvas.winfo_height(), 200)  # Prevents zero or negative height

        # Generate random position within safe bounds
        x = random.randint(50, max(51, canvas_width - 50))  # Ensures valid range
        y = random.randint(20, max(21, min(150, canvas_height - 50)))

        # Animate glow by cycling through colors
        glow_colors = ["#FF0000", "#FF4500", "#FF6347", "#FF7F50", "#FFA500"]
        self.glow_index = (self.glow_index + 1) % len(glow_colors) if hasattr(self, 'glow_index') else 0
        glow_color = glow_colors[self.glow_index]

        # Clear previous text before drawing new one
        self.canvas.delete("n_value_text")

        # Create the glow effect by layering texts with slight offsets
        ...
        # Main text in white for readability
        self.canvas.create_text(
            x, y,
            text=f"Current N Value: {self.n_value}",
            fill="white",
    ...
    def display_same_shape(self):
        ...
        self.handle_response(self.check_match())
        self.display_random_shape()
    ...
    def display_not_same_shape(self):
        ...
        self.rotation_angle = random.randint(0, 360)
        self.handle_response(not self.check_match())
        self.display_random_shape()
        self.canvas.after(400, lambda: self.not_same_button.configure(state="normal"))

    def handle_response(self, is_correct):
        ...
        print(f"N value: {self.n_value}, Accuracy: {accuracy:.2f}%, Reaction Time: {reaction_time:.4f}s, Error Ratio: {error_ratio:.2f}")
        self.csv_writer.writerow([accuracy, reaction_time, error_ratio])
...
class IntroWindow(ctk.CTk):
    ...
    def add_initial_buttons(self):
        # Create a frame for the initial buttons (ANT and ST)
        ...
        self.ant_button.grid(row=0, column=0, padx=10)
        ...
        self.st_button.grid(row=0, column=1, padx=10)

    def dont_add_anything(self):
        # This function is called when the "ST" button is pressed
        self.destroy()
        app = NBackApp()  # Creating an instance
        app.mainloop()
        exit()

    def show_login_or_register(self):
        # Remove the ANT and ST buttons
        ...
        self.back_button.grid(row=0, column=2, padx=10, sticky="ne")

    def go_back(self):
        self.buttons_frame.destroy()
        self.add_initial_buttons()  # Add the initial buttons again
    ...
    def on_space_press(self, event):
        self.buttons_frame.destroy()
         # Create a frame for buttons (Login and Sign Up)
        if self.buttons_frame is None:  # Ensure buttons are only created once
            self.buttons_frame = ctk.CTkFrame(self)
            self.buttons_frame.pack(pady=20)
...
class MainApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.wm_title("Shape Display Application")
        self.geometry("800x600")
        self.minsize(600, 500)
    ...
    def draw_shape(self, shape_type, size, angle, color):
        self.canvas.create_text(
                430, 25,  # Coordinates for the text (centered)
                text=f"Current N Value : {self.n_value} ",
                fill="#1E90FF",
                font=("Arial", 14)
            )
    ...
    def check_response(self, is_correct):
        reaction_time = time.time() - self.start_time
        self.total_count += 1
        if is_correct:
            self.correct_count += 1
        ...
        error_ratio = self.incorrect_responses / self.total_count
        print(f"N value: {self.n_value}, Accuracy: {accuracy:.2f}%, Reaction Time: {reaction_time:.4f}s, Error Ratio: {error_ratio:.2f}, accuracy_treds: {accuracy_treds : .2f},correct_responses: {self.correct_count}, incorrect_responses:{self.incorrect_responses}, total_responses : {self.total_count}, Result : {is_correct}")
    ...
    def check_match(self):
        if len(self.shape_history) <= self.n_value:
            return False  # Not enough history to compare
        #print(self.shape_history[-1] == self.shape_history[-(self.n_value + 1)])
        return self.shape_history[-1] == self.shape_history[-(self.n_value + 1)]

    def display_same_shape(self):
        self.same_button.configure(state="disabled")
        self.rotation_angle = random.randint(0, 360)  # Random rotation angle
        self.check_response(self.check_match())
        self.display_random_shape()
        self.canvas.after(400, lambda: self.same_button.configure(state="normal"))
    ...
    def display_not_same_shape(self):
        self.not_same_button.configure(state="disabled")
        self.rotation_angle = random.randint(0, 360)  # Random rotation angle
        self.check_response(not self.check_match())
        self.display_random_shape()
        self.canvas.after(400, lambda: self.not_same_button.configure(state="normal"))
...
class P(MainApp):
    ...
    def display_shapes(self,total_shapes):
        if self.shape_count < total_shapes:
            ...
            self.after(2500, self.display_shapes, total_shapes)
            print(f'self.shape_history_practice: {self.shape_history_practice}')
            res = self.check_shape_match()
            #print(f'res: {res}')
            
            #self.canvas.create_text(
            #    350 ,40,
            #    text=f"Displaying Shape:{self.shape_count} And MatchIng with N={self.n_value}: {res}",
            #    fill='#1E90FF',
            #    font=("Arial", 18, "bold"),
            #    tags="n_value_text"
            #)
    ...
    def check_shape_match(self):
        if len(self.shape_history_practice) <= self.n_value:
            return False  # Not enough history to compare
        print(self.shape_history_practice[-1] == self.shape_history_practice[-(self.n_value + 1)])
